...
(async () => {
  ...
  const r = await c.query("SELECT output::text AS o, failure_summary, verdict, created_at, verification_id FROM run WHERE short_id=$1", [sid]);
  const o = JSON.parse(r.rows[0].o);
  console.log('RUN', sid, '| verdict:', r.rows[0].verdict, '| at:', r.rows[0].created_at, '| ver_id:', r.rows[0].verification_id, '| dur:', (o.events||[]).length, 'events');
  console.log('FAIL_SUMMARY:', (r.rows[0].failure_summary||'-').slice(0,300));
  const evs = (o.events||[]).slice(-8);
  for (const e of evs) console.log('EV', e.type, '|', String(e.stepId||''), '|', (e.detail||e.title||JSON.stringify(e.args||{})).slice(0,160));
  await c.end();
