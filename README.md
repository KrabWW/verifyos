...
```bash
bash scripts/setup-llm.sh   # ① 选提供商（智谱 GLM / DeepSeek / 自定义 OpenAI 兼容）
                            # ② 粘贴 API Key → 自动生成 .env（权限 600）
bash start.sh               # ③ 本地一键起服；或走下方 docker 私有化部署
```
...
```bash
bash scripts/setup-llm.sh   # 交互式生成 .env（或手动 cp .env.example .env 后按注释填写）
